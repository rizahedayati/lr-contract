...
const deploy_lrt_team_vesting_second = require("./deploy_scripts/deploy_lrt_team_vesting_second");
...
let lrtVestingTeamSecondFixtureData;
...
async function lrtVestingTeamSecondFixture() {
  const [
    owner,
    admin,
    distributor,
  ...
  const lrtVestingTeamSecondInstance = await deploy_lrt_team_vesting_second(lrtDistributorInstance,arInstance);

  await arInstance.grantRole(
    APPROVED_CONTRACT_ROLE,
    lrtVestingTeamSecondInstance.address
  );

  await arInstance.grantRole(DISTRIBUTOR_ROLE, lrtDistributorInstance.address);

  lrtVestingTeamSecondFixtureData = {
  ...
  return lrtVestingTeamSecondFixtureData;
}

    ...
    IAssetMarketplace
{
    ...
    bytes32 public constant FULL_FILL_ORDER_SIGN =
        keccak256(
            "fullFillOrder(address seller,bytes16 assetName,uint64 expireDate,uint256 price,uint256 quantity)"
        );
     ...
     * @param _sellUnit The quantity that needs to check.
     */
    modifier validQuantity(uint256 _sellUnit) {
    ...
    function buyItem(uint256 _sellId) external override nonReentrant {
        ...
        if (hasSufficientBalance) {
            _processTokenPurchase(_sellId, msg.sender, price, assetSell);
        } else {
            _proccessVestingPurchase(_sellId, msg.sender, price, assetSell);
        }
    ...
    function _processTokenPurchase(
        ...
        address _buyer,
        uint256 _price,
        AssetSell memory _sellOrder
    ) private {
        require(
            lrt.allowance(_buyer, address(this)) >= _price,
            "AssetMarketplace::Allowance error"
        );
        emit AssetBoughtWithBalance(
            _sellId,
            _sellOrder.assetName,
            msg.sender,
            _sellOrder.sellUnit,
            _price
        );
        require(
            lrt.transferFrom(_buyer, address(this), _price),
            "AssetMarketplace::Unsuccessful transfer"
        );
    }

    function _proccessVestingPurchase(
        uint256 _sellId,
        address _buyer,
        uint256 _price,
        AssetSell memory _sellOrder
        ...
        lrtVesting.setDebt(_buyer, _price);
    }
